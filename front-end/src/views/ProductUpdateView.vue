  ...
  <div class="flex flex-col max-w-4xl justify-center w-full">
    ...
    <div class="flex flex-col max-w-4xl w-full bg-white items-center rounded-b-xl p-4 shadow-xl">
      ...
      <div class="flex w-full justify-end mt-1">
        <button @click="update" class="bg-red-600 hover:bg-red-400 py-4 px-10 text-white rounded mr-1 text-xl ">Update Product</button>
      </div>
    </div>
  </div>
</template>
...
const update = async () => {
  try {
    ...
    const data = await axios.put(url + productId, params, {
      headers: {
